...
struct electorateExperiment {
	...
	int initialWinner, initialWinnerRoundCount, initialOverallHappiness = 0;
...
/* 		case 5:{
	...
	int totalTotal = 0, totalTotalUps = 0, totalTotalDowns = 0, totalTotalRounds = 0, totalTotalStable = 0;
	while(needsFirstRun || needsSecondRun)
	{
		int strategyOverallHappiness = 0;
		...
		for(unsigned electorateCount = 0; electorateCount < manyElectorates.size(); electorateCount++)
		{
			...
			//	vector<int> winners,individualAdjustments;
				...
				experimentReference.initialOverallHappiness	= 0;
			}
			for(unsigned voterIndex = 0; voterIndex < VOTER_COUNT; voterIndex++)
			{
				experimentReference.initialOverallHappiness += experimentReference.theseVoters.at(voterIndex).preferenceAt(experimentReference.initialWinner);
			}
			strategyOverallHappiness += experimentReference.initialOverallHappiness;
	...
	/* 	for(pollMethod poll2ToTry = FIRST_STRAT; poll2ToTry < STRAT_MAX; poll2ToTry = pollMethod(poll2ToTry + 1))
		...
		researchResults << "\nFrom: " << stratToString(firstPollMethod,firstPairMethod,firstBuilderMethod) << " Happiness: " << strategyOverallHappiness << "\n";
		...
		for(unsigned voterIndex = 0; voterIndex < VOTER_COUNT; voterIndex++)
		{
			for(unsigned electorateCount = 0; electorateCount < manyElectorates.size(); electorateCount++)
			{
				...
				totalTotalRounds += experimentReference.roundsToFinish.at(voterIndex);
				...
				//I don't know what this number below actually does. Probably bad code.
				totalStable += (experimentReference.electionWasStable.at(voterIndex) ? 1 : 0);
				totalTotalStable += (experimentReference.electionWasStable.at(voterIndex) ? 1 : 0);
				//I don't know what this number above actually does.
		...
		for(unsigned electorateCount = 0; electorateCount < manyElectorates.size(); electorateCount++)
		{
			...
			researchOutput << left << " - Initial Winner: " << static_cast<char>(experimentReference.initialWinner + 'A') 
				<< " Happiness: " << experimentReference.initialOverallHappiness
		...
		researchResults << "====\nTotal : " << totalResult << " Ups: " << totalUps << " Downs: " <<totalDowns << " Rounds: " << totalRounds << " Stable Elections: " << totalStable << "\n====\n";
		if(needsFirstRun)
		{
			needsFirstRun = false;
			totalTotalUps = totalUps;
			totalTotalDowns = totalDowns;
		...
		{
			totalTotalUps += totalDowns;
			totalTotalDowns += totalUps;
			needsSecondRun = false;
			researchResults << "TotalTotal : " << totalTotal << " TotalTotalUps: " << totalTotalUps << " TotalTotalDowns: " << totalTotalDowns << "\n";
		}
	};
	if(PRINT_TO_FILE)
	{
