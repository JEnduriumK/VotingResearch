...
void debugOutput(string const &theString,const int &debugLevel)
{//Gatekeeping the debugOutput(string) function behind a debugLevel, so I don't have to bake if() statements in to all my debug lines in the main code.
	if((DEBUG_MODE & debugLevel) == debugLevel)
		debugOutput(theString);
}
void debugOutput(stringstream const&theStringStream,const int&debugLevel)
{
	if((DEBUG_MODE & debugLevel) == debugLevel)
		debugOutput(theStringStream.str());
}
...
//https://urldefense.com/v3/__https://stackoverflow.com/questions/1577475/c-sorting-and-keeping-track-of-indexes__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkylr5fqk$ 
...
string Election::toString() const
{
	...
	switch(this->getElectionType()){
		...
		case hare:
		{
			...
			//https://urldefense.com/v3/__https://www.tutorialspoint.com/determining-how-many-digits-there-are-in-an-integer-in-cplusplus__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkQDNgelo$ 
			break;
		}
		case copeland:
		{
	...
	for(unsigned int index = 0; index < theElectorate.size(); index++)
	{
		...
		if(oldNormalizedBallot != newNormalizedBallot)
		{
			...
			*/
			...
			ballotsChanged = true;
			debugOutput("Voter " + to_string(index) + " changed vote from " + vectorString(oldNormalizedBallot) + " to " + vectorString(newNormalizedBallot) + '\n',16);
			//https://urldefense.com/v3/__https://stackoverflow.com/questions/44807347/using-stdaccumulate-on-a-two-dimensional-stdarray__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHknoyUXdw$ 
			/*
			...
			*/
			...
			//https://urldefense.com/v3/__https://stackoverflow.com/questions/3752019/how-to-get-the-index-of-a-value-in-a-vector-using-for-each__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHk-DyZawM$ 
...
vector<int> Election::currentPoll() const
{
	...
	//https://urldefense.com/v3/__https://cplusplus.com/reference/algorithm/transform/__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkRadZUwU$ 
...
#define HARE_PAIRWISE_JUST_VICTORIES
#ifdef HARE_PAIRWISE_JUST_VICTORIES
			for (unsigned int pairwiseIndex = 0;pairwiseIndex < evaluationPoll.size(); pairwiseIndex++)
...
#endif // HARE_PAIRWISE_JUST_VICTORIES
//#define HARE_PAIRWISE_JUST_WIN_OR_LOSE
#ifdef HARE_PAIRWISE_JUST_WIN_OR_LOSE
			for (unsigned int pairwiseIndex = 0;pairwiseIndex < evaluationPoll.size(); pairwiseIndex++)
			{//Ignoring ballots, go through each candidate in the current evaluation of the election and...
				if (evaluationPoll.at(pairwiseIndex) <= this->ballots.size()) // or could use < instead of <=
				{//If they don't have a score that is at least half that of the winner's score...? Zero them out?
					evaluationPoll.at(pairwiseIndex) = 0;
				}
				else if (evaluationPoll.at(pairwiseIndex) < evaluationPoll.at(currentWinner))
				{//If they're at half-or-better, put them at half?
					evaluationPoll.at(pairwiseIndex) = this->ballots.size();
				}
			}
#endif // HARE_PAIRWISE_JUST_WIN_OR_LOSE
#endif // ADD_HARE_EVAL
#ifdef SUBTRACT_HARE_EVAL
			fill(evaluationPoll.begin(),evaluationPoll.end(),this->ballots.size() * 2 + 1);
			//debugStream << "Pairwise Evaluation - Current Winner: " << static_cast<char>(currentWinner+'A') << " (worth " << this->ballots.size() * 2/*  + 1  */<< ") ";
			for(unsigned int ballotIndex = 0; ballotIndex < this->ballots.size(); ballotIndex++)
			{
				for(unsigned int pairwiseIndex = 0;pairwiseIndex < evaluationPoll.size(); pairwiseIndex++)
				{
					...
					if(this->ballots.at(ballotIndex).at(pairwiseIndex) == this->ballots.at(ballotIndex).at(currentWinner))
					{
						evaluationPoll.at(pairwiseIndex)--;
					}
				}
			}
			for(unsigned int pairwiseIndex = 0; pairwiseIndex < evaluationPoll.size(); pairwiseIndex++)
			{
				if(evaluationPoll.at(pairwiseIndex) < evaluationPoll.at(currentWinner))
				{
					evaluationPoll.at(pairwiseIndex)--;
				}
			}
		...
		{
			//https://urldefense.com/v3/__https://stackoverflow.com/questions/8833559/can-i-assign-in-2-variables-at-the-same-time-in-c__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkis5GIcQ$ 
...
tuple<unsigned int,vector<vector<int>>> Election::runSchultze() const
{
	...
	for(unsigned candidateA = 0; candidateA < this->getNumCandidates(); candidateA++)
	{
		for(unsigned candidateB = 0; candidateB < this->getNumCandidates(); candidateB++)
		{
			if(candidateA == candidateB)
				continue;
			//https://urldefense.com/v3/__https://en.wikipedia.org/wiki/Schulze_method__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkOfdiCGg$  ?!?!?
		...
		case coombs:
			...
			*/
			...
			for(unsigned int index = 0; index < this->ratings.size(); index++)
			{
				...
				countSum = 0;
				//https://urldefense.com/v3/__https://en.cppreference.com/w/cpp/algorithm/count__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkvrQf9aA$ 
				//unordered_map.insert({this->ratings.at(index),count(this->ratings.begin(),this->ratings.end(),this->ratings.at(index))});
				//https://urldefense.com/v3/__https://en.cppreference.com/w/cpp/utility/functional/less__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkRnSynjo$  ?? Nahh.
				...
				//https://urldefense.com/v3/__https://stackoverflow.com/questions/57151983/counting-elements-greater-than-a-number-in-vector__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkrzLlrBY$ 
			...
			//https://urldefense.com/v3/__https://en.cppreference.com/w/cpp/container/unordered_map/insert__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHk1X_kw9M$ 
			
			/* vector<int> sortedRatings = this->ratings;
			//https://urldefense.com/v3/__https://en.cppreference.com/w/cpp/algorithm/sort__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkKmEeg48$ 
...
string Voter::toString() const
{
	//string returnValue;
	stringstream tempStream;
	tempStream << "TruePreferences: {" << right;
...
/* 	for(int index = 0; index < doubleBordaPoll.size(); index++)
	...
	switch(this->pairingStrategy)
	{
		...
		case neighbors:
			for(unsigned int gap = 1;gap < currentPoll.size();gap++)
			{
				...
				{
					thePairings.push_back(make_tuple(frontPair,backPair,
						currentPoll.size() - (backPair - frontPair) * exceedingValue + max(doubleBordaPoll.at(frontPair),doubleBordaPoll.at(backPair))
					));
				}
			}
			break;
	...
	//https://urldefense.com/v3/__https://stackoverflow.com/questions/23030267/custom-sorting-a-vector-of-tuples__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkAS_7UDk$ 
...
vector<int> Voter::generateBallot(const Election&thisElection) const
{
	...
	stringstream debugStream;
	//int maxCounter = MAX_RATING;
	//int minCounter = MIN_RATING;
	int maxCounter = MAX_RATING+1;
	int minCounter = MIN_RATING-1;
	// switch(this->getStrengthEval())
	// {
		// case strat_allZero:
			// fill(currentPoll.begin(),currentPoll.end(),0);
			// break;
		// case strat_sentinelValue:
			// debugOutput("ERROR: You reached strat_sentinelValue at " + to_string(__LINE__) + " in vote.cpp, aborting.\n",1);
			// exit(3);
	...
	*/
	...
	for(unsigned int index = 0; index < thePairings.size(); index++)
	{
		...
		*/
		...
		{
			//https://urldefense.com/v3/__https://stackoverflow.com/questions/4527686/how-to-update-stdmap-after-using-the-find-method__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkK4h5_HE$ 
			//https://urldefense.com/v3/__https://stackoverflow.com/questions/2551775/appending-a-vector-to-a-vector__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHkwS-9tFU$ 
	...
	//https://urldefense.com/v3/__https://en.cppreference.com/w/cpp/container/map__;!!KGfBWX4!ssX8FwCERDl__uiz_p4pRex2V0kNVyw8L7j3vl9F5BTT_CPa86fxodcxC5ECfEygwAFBcIEKeTZ2ZBHk0NWmTjg$ 
	if((DEBUG_MODE & 4) == 4)
	{
	...
	// for(unsigned int pairIndex = 0; pairIndex < thePairings.size(); pairIndex++)
	// {
		// indexOne = get<0>(thePairings.at(pairIndex));
		// indexTwo = get<1>(thePairings.at(pairIndex));
		// if(this->getBuildMethod() == bothEnds || this->getBuildMethod() == front)
		// {
			// if(this->preferences.at(indexOne) >= this->preferences.at(indexTwo))
			// {
				// if(buildABallot.at(indexOne) == -1)
					// buildABallot.at(indexOne) = maxCounter--;
			// }
			// else
			// {
				// if(buildABallot.at(indexTwo) == -1)
					// buildABallot.at(indexTwo) = maxCounter--;
			// }
		// }
		// if(this->getBuildMethod() == bothEnds || this->getBuildMethod() == back)
		// {
			// if(this->preferences.at(indexOne) < this->preferences.at(indexTwo))
			// {
				// if(buildABallot.at(indexOne) == -1)
					// buildABallot.at(indexOne) = minCounter++;
			// }
			// else
			// {
				// if(buildABallot.at(indexTwo) == -1)
					// buildABallot.at(indexTwo) = minCounter++;
			// }
		// }
	// }
	// unsigned int groupingSum, lastValueWas;
	unsigned int groupingSum;
	unsigned int candidateIndex;
	//int groupingSize, disapprovalCounter;
	int groupingSize;
	...
	//int candidateWeAreConsideringByPreference;
	...
	/* vector<int> disapprovalList(this->preferences.size(),-1);
	for (map<int, vector<pair<int,int>>>::const_reverse_iterator mapIterator = theGroupings.crbegin(); mapIterator != theGroupings.crend() && count(buildABallot.begin(),buildABallot.end(),-1) > 0; mapIterator++)
	{
		...
		lastValueWas = INT_MAX;
		disapprovalCounter = 0;
		fill(disapprovalList.begin(),disapprovalList.end(),-1);
		groupingSum = 0;
		...
		for(const int&candidateWeAreConsideringByPreference : sortedCandidateIndexes)
		{
			...
			{
				debugOutput("Candidate " + to_string(sortedCandidateIndexes.at(candidateWeAreConsideringByPreference)) + " already has " + to_string(buildABallot.at(candidateWeAreConsideringByPreference)) + ", skipping.\n",4);
				continue;
			}
			...
			if(buildABallot.at(candidateWeAreConsideringByPreference) == -1)
			{
				...
				if(lastValueWas == INT_MAX)
				{
					lastValueWas = thisCandidateValue;
				}
				debugOutput("Sum:" + to_string(groupingSum) + "\n",4);
				if(buildFromFront && (thisCandidateValue > groupingSum))//More weird averaging!
				{
					buildABallot.at(candidateWeAreConsideringByPreference) = (lastValueWas != thisCandidateValue ? --maxCounter : maxCounter);
					debugOutput("Approve because candidate " + string(1,static_cast<char>(candidateWeAreConsideringByPreference+'A')) + 
						...
						", assigning " + to_string(buildABallot.at(candidateWeAreConsideringByPreference)) + "\n",4);
				}
				else if(buildFromBack && (thisCandidateValue < groupingSum))
				{
					disapprovalList.at(candidateWeAreConsideringByPreference) = (lastValueWas != thisCandidateValue ? ++disapprovalCounter : disapprovalCounter);
					debugOutput("Disapprove: because candidate " + string(1,static_cast<char>(candidateWeAreConsideringByPreference+'A')) + 
						" is 'worth' " + to_string(this->preferences.at(candidateWeAreConsideringByPreference) * candidatesToConsider.size()) + 
						", assigning " + to_string(disapprovalList.at(candidateWeAreConsideringByPreference)-1)+ " (disapprovals in same group will invert later)\n",4);
				}
				else
				{
		...
		debugOutput("DisList: " + vectorString(disapprovalList) + "disapprovalCounter at " + to_string(disapprovalCounter) + ", minCounter at " + to_string(minCounter) + "\n",128);
		for(unsigned int index = 0; index < disapprovalList.size(); index++)
		{//Time to invert those low values, so 2,1,0 would become 0,1,2.
			if(disapprovalList.at(index) >= 0)
			{
				disapprovalList.at(index) = (disapprovalCounter) - disapprovalList.at(index);
			}
		}
		debugOutput("DisList Inversion: " + vectorString(disapprovalList) + "disapprovalCounter at " + to_string(disapprovalCounter) + ", minCounter at " + to_string(minCounter) + "\n",128);
		transform(disapprovalList.begin(),disapprovalList.end(),disapprovalList.begin(),[minCounter](int theValue){
			return (theValue > 0 ? theValue + minCounter: theValue);
		});
		debugOutput("DisList Transform: " + vectorString(disapprovalList) + "disapprovalCounter at " + to_string(disapprovalCounter) + ", minCounter at " + to_string(minCounter) + "\n",128);
		minCounter += disapprovalCounter+1;
		for(unsigned int index = 0; index < disapprovalList.size(); index++)
		{
			if(disapprovalList.at(index) >= 0)
			{
				buildABallot.at(index) += (disapprovalList.at(index)+1);
			}
		}
		debugOutput("DisList Builder: " + vectorString(disapprovalList) + "disapprovalCounter at " + to_string(disapprovalCounter) + ", minCounter at " + to_string(minCounter) + "\n",128);
	}
	 */
	...
	for(unsigned int index = 0;index < buildABallot.size(); index++)
	{//Anything that didn't get assigned a value now finally gets assigned a default.
		if(buildABallot.at(index) >= 0)
		{
		...
		else
		{
			...
			{
				case bothEnds:
					buildABallot.at(index) = (MAX_RATING + MIN_RATING) / 2;
					break;
				case front:
					buildABallot.at(index) = minCounter++;
					break;
				case back:
					buildABallot.at(index) = maxCounter--;
					break;
