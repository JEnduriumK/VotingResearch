...
/* 		case 5:{
	...
	for(unsigned electorateCount = 0; electorateCount < ELECTORATE_COUNT; electorateCount++)
	{
		...
		if(DEBUG_MODE == 256)
		{
			...
			for(;electorateCount < ELECTORATE_COUNT; electorateCount++)
			{
				rotate(tempExperimentContainer.theseVoters.begin(),tempExperimentContainer.theseVoters.begin()+1,tempExperimentContainer.theseVoters.end());
...
//	}
	...
	/* 	for(pollMethod poll2ToTry = FIRST_STRAT; poll2ToTry < STRAT_MAX; poll2ToTry = pollMethod(poll2ToTry + 1))
		...
		for(unsigned electorateCount = 0; electorateCount < manyElectorates.size(); electorateCount++)
		{
			...
			researchOutput << vectorString(voterIDiota,3) << " - VoterID\n";
			...
			researchOutput << vectorString(experimentReference.roundsToFinish,3) << " - Rounds\n";
			researchOutput << vectorString(experimentReference.electionWasStable,3) << " - Stable\n";
		...
		researchResults << vectorString(voterIDiota,4) << " - VoterID\n";
		...
		researchResults << vectorString(totalRoundsVector,4) << " - Total Rounds To Finish\n";
		researchResults << vectorString(totalRoundsWhenStableVector,4) << " - Total Rounds When Stabilizing\n";
