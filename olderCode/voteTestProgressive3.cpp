...
const string PROGRESSION_TYPE = "P";
...
int main(int argc, char *argv[])
{
	...
	for(tempElectionInput = 1;tempElectionInput < static_cast<int>(numElectionTypes); tempElectionInput++)
	for(tempPollInputX = 1;tempPollInputX < static_cast<int>(strat_sentinelValue); tempPollInputX++)
	for(tempPairInputX = 1;tempPairInputX < static_cast<int>(pair_sentinelValue); tempPairInputX++)
	for(tempBuildInputX = 1;tempBuildInputX < static_cast<int>(build_sentinelValue); tempBuildInputX++)
	for(tempPollInputY = static_cast<int>(strat_sentinelValue)-1;tempPollInputY > tempPollInputX; tempPollInputY--)
	for(tempPairInputY = static_cast<int>(pair_sentinelValue)-1;tempPairInputY > tempPairInputX ; tempPairInputY--)
	for(tempBuildInputY = static_cast<int>(build_sentinelValue)-1;tempBuildInputY > tempBuildInputX ; tempBuildInputY--)
	//for loops intentionally without {}.
	{
...
/* 			case 5:{
	...
	/* 	for(pollMethod poll2ToTry = FIRST_STRAT; poll2ToTry < STRAT_MAX; poll2ToTry = pollMethod(poll2ToTry + 1))
		...
		resultsFile.close();
		cout << "Generated: " << outputFileName.str() << "\n";
		if(PRINT_DETAILS)
		{
		...
		outputFileName.str(string());
		researchResults.str(string());
		researchOutput.str(string());
	}
	//srandom(time(0));
	//cout << "Seed: " << time(0) << "\n";
